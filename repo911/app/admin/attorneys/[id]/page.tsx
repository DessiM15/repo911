  ...
  useEffect(() => {
    async function fetchAttorney() {
      try {
        ...
        if (!res.ok) {
          toast.error('Failed to load attorney');
          return;
        }
    ...
    <div className="max-w-5xl mx-auto space-y-4">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">Firm</span><span className="dark:text-gray-200">{attorney.firm_name || 'N/A'}</span></div>
            <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">Bar Number</span><span className="dark:text-gray-200">{attorney.bar_number}</span></div>
            <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">Bar State</span><span className="dark:text-gray-200">{attorney.bar_state}</span></div>
            <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">Licensed States</span><span className="dark:text-gray-200">{attorney.licensed_states?.join(', ') || 'N/A'}</span></div>
            <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">Preferred States</span><span className="dark:text-gray-200">{attorney.preferred_states?.join(', ') || 'N/A'}</span></div>
            <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">Case Types</span><span className="dark:text-gray-200">{attorney.preferred_case_types?.join(', ') || 'N/A'}</span></div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              <span className="text-gray-500 dark:text-gray-400">Lead Purchase Agreement</span>
            ...
            {attorney.fee_agreement_signed_at && (
              <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">Signed At</span><span className="dark:text-gray-200">{formatDate(attorney.fee_agreement_signed_at)}</span></div>
            )}
            <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">Stripe Customer</span><span className={attorney.stripe_customer_id ? 'text-green-600' : 'text-gray-400 dark:text-gray-500'}>{attorney.stripe_customer_id ? 'Connected' : 'Not connected'}</span></div>
            <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">Email Notifications</span><span className="dark:text-gray-200">{attorney.email_notifications ? 'On' : 'Off'}</span></div>
            <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">SMS Notifications</span><span className="dark:text-gray-200">{attorney.sms_notifications ? 'On' : 'Off'}</span></div>
            <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">Registered</span><span className="dark:text-gray-200">{formatDate(attorney.created_at)}</span></div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden">
        ...
        {claimedLeads.length === 0 ? (
          <div className="p-8 text-center text-gray-400 dark:text-gray-500">No claimed leads yet.</div>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200 dark:border-slate-700">
          <h3 className="font-semibold text-gray-900 dark:text-gray-100">Transaction History ({transactions.length})</h3>
        </div>
            ...
            <TableBody>
              {transactions.map((tx) => (
                <TableRow key={tx.id}>
                  ...
                  <TableCell className="text-sm text-gray-500 dark:text-gray-400">{tx.description || '—'}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
